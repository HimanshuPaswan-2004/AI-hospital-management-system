...
export const generateInvoicePDF = (invoice) => {
  ...
  doc.text(`Appointment Date: ${new Date(invoice.appointment.appointmentDate).toLocaleDateString()}`, 120, 78);
  ...
  tableData.push([
    ...
    `$${invoice.consultationFee.toFixed(2)}`
  ]);

  // 2. Pharmacy Charges
  if (invoice.appointment.prescription && invoice.appointment.prescription.medicines) {
    const meds = invoice.appointment.prescription.medicines;
    meds.forEach(med => {
      tableData.push([
        'Medicine (Prescribed)',
  ...
  tableData.push([
    'Pharmacy Total',
    '',
    `$${invoice.pharmacyCharges.toFixed(2)}`
  ]);
  ...
  doc.text(`$${invoice.totalAmount.toFixed(2)}`, 175, finalY);
