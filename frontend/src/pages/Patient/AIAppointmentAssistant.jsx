                  ...
                  <div className="space-y-4">
                     {resultData?.slots?.length > 0 ? (
                       resultData.slots.map((slot, idx) => (
                         <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors group">
                            <div className="flex items-center gap-4">
                               <div className="w-10 h-10 rounded-full bg-slate-200 border-2 border-white shadow-sm flex items-center justify-center overflow-hidden">
                                  <span className="text-slate-600 font-bold text-sm">{slot.doctorName.split(' ').map(n=>n[0]).join('').substring(0,2)}</span>
                               </div>
                               <div>
                                  <h4 className="font-bold text-slate-800 text-sm">{slot.doctorName}</h4>
                                  <p className="text-[11px] font-medium text-slate-500">{slot.specialization}</p>
                               </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between sm:justify-end gap-6 sm:w-1/2">
                               <div className="flex flex-col gap-1">
                                  <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700">
                                     <Calendar size={14} className="text-blue-600" /> {slot.date}
                                  </div>
                                  <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700">
                                     <Clock size={14} className="text-blue-600" /> {slot.time}
                                  </div>
                               </div>
                       ...
                       <p className="text-sm font-medium text-slate-500 bg-slate-50 p-4 rounded-xl text-center">No slots found matching your request.</p>
                     )}
                  </div>
               </div>
            </div>
