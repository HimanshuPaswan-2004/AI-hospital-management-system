...
const AIAssistantHome = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">AI Health Assistant</h1>
        <p className="text-slate-500 mt-2 text-[15px] font-medium">How can I help you today?</p>
      </div>
        ...
        {tools.map((tool, index) => {
          ...
          return (
            <Link key={index} to={tool.link} className="block group">
              <div className="pro-card p-6 h-full transition-all duration-300 hover:border-blue-200 hover:shadow-lg hover:shadow-blue-500/5 hover:-translate-y-1">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-5 ${tool.bg} group-hover:scale-110 transition-transform duration-300`}>
                  <Icon className={`w-6 h-6 ${tool.color}`} />
                </div>
                
                <h3 className="text-[17px] font-bold text-slate-800 mb-2 group-hover:text-blue-600 transition-colors">
                  {tool.title}
                </h3>
